...
public class IndexService {
    ...
    @Autowired
    private GoodsClient goodsRepository;
  ...
  public Goods buildGoods(SpuBo spuBo) {
    ...
    //再查sku
    List<Sku> skus = goodsClient.querySkuListBySpuId(spuBo.getId());
    //进行属性拷贝
    BeanUtils.copyProperties(spuBo, goods);
    //封装all字段
    String title = spuBo.getTitle();
    //封装cnames
    List<String> cnames = this.categoryClient.queryNamesByIds(Arrays.asList(spuBo.getCid1(), spuBo.getCid2(), spuBo.getCid3()));
    ...
    List<SpecParam> specParams = this.specificationClient.queryParamsByGid(null, spuBo.getCid3(), true, null);
